...
def range(request):
    ...
    try:
        ...
        # Handle the case where the values are not valid floats
        # You may want to redirect the user or display an error message
        return HttpResponse("Invalid values for min and/or max")

    q1 = Q(pcost__gte=min_value) if min_value is not None else Q()
    q2 = Q(pcost__lte=max_value) if max_value is not None else Q()
    q3 = Q(is_active=True)
...
def sendusermail(request):
        ...
        msg,
        "chepurikavyasree2001@gmail.com",
        [uemail],
        fail_silently=False,
